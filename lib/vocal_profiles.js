...
const VOCAL_STYLE_PROFILES = {
    ...
    "scat_riff": {
        style_label: "scat_riff_expressive",
        interval_pattern: [
            { interval: 0,  probability: 0.25 },
            { interval: 3,  probability: 0.20 },
            { interval: 5,  probability: 0.20 },
            { interval: 7,  probability: 0.15 },
            { interval: -2, probability: 0.10 },
            { interval: 4,  probability: 0.10 }
        ],
        direction_bias: { up: 0.3, down: 0.3, same: 0.1, erratic: 0.3 },
        note_duration_rules: [
            { duration_type: 'eighth',    probability: 0.5, relative_to_beat: true },
            { duration_type: 'quarter',   probability: 0.3, relative_to_beat: true },
            { duration_type: 'sixteenth', probability: 0.2, relative_to_beat: true }
        ],
        rest_rules: [
            { duration_type: 'eighth',     probability: 0.20, relative_to_beat: true },
            { duration_type: 'sixteenth',  probability: 0.15, relative_to_beat: true },
            { duration_type: 'pause_skip', probability: 0.50 }
        ],
        rhythm_accent_rules: {
            on_beat: 0.2, off_beat_sync: 0.5, complex_sync: 0.3,
            unpredictable_syncop_prob: 0.5
        },
        velocity_rules: { base: 78, range: 20, accent_modifier: 12 },
        mode_preference: null,
        vocal_register_rules: {
            preferred_octave: 4, min_midi: 55, max_midi: 74
        }
    }
};
