...
let _arpeggioConsecutiveSilences = 0;
let _arpeggioLastSectionType = null;
...
function generateChordRhythmEvents(songMidiData, CHORD_LIB_GLOBAL, NOTE_NAMES_GLOBAL, helpers, slotContext) {
    ...
    // FIX 3: phrase-level rests — chance to skip entire slot
    if (slotContext.sectionType !== _arpeggioLastSectionType) {
        _arpeggioConsecutiveSilences = 0;
        _arpeggioLastSectionType = slotContext.sectionType;
    }
    const silenceChance = isVerse ? 0.25 :
                          isBridge ? 0.40 :
                          isIntro ? 0.30 :
                          isOutro ? 0.50 : 0.10;
    const effectiveSilenceChance = _arpeggioConsecutiveSilences >= 2 ? 0 : silenceChance;
    if (Math.random() < effectiveSilenceChance) {
        _arpeggioConsecutiveSilences++;
        return rhythmicEvents; // empty
    }
    _arpeggioConsecutiveSilences = 0;
    ...
    while (currentTickInSlot < durationTicks) {
        ...
        if (durationInBeats > 0) {
            ...
            rhythmicEvents.push({
                ...
                startTick: humanizeTiming(startTickAbsolute + currentTickInSlot, 4),
